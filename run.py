...
def create_gif_plot(feature_key, safe_features_3d, unsafe_features_3d, pca):
        # Create rotating GIF
        print(f"Creating rotating GIF for {feature_key}")
        angles = np.linspace(0, 360, 240)  # 240 frames for smooth rotation
        gif_frames = []
...
def flatten_safe_unsafe_features(extracted_safe_unsafe_diffusion_features_per_sample, feature_key):

    flattened_safe_features = []
    flattened_unsafe_features = []
    for sample_id, value in extracted_safe_unsafe_diffusion_features_per_sample.items():
        ...
        flattened_safe_features.append(safe_flat)
        flattened_unsafe_features.append(unsafe_flat)

    return flattened_safe_features, flattened_unsafe_features


def get_pca_features(flattened_safe_features, flattened_unsafe_features, n_components=3):
    """
    Get PCA features for safe and unsafe clusters
    
    Args:
        flattened_safe_features: List of flattened safe features
        flattened_unsafe_features: List of flattened unsafe features
    ...
    safe_features_matrix = np.vstack(flattened_safe_features)  # [n_samples, n_features]
    unsafe_features_matrix = np.vstack(flattened_unsafe_features)
    ...
    safe_features_nd = pca_features[:n_safe]
    unsafe_features_nd = pca_features[n_safe:]

    return pca_features, safe_features_nd, unsafe_features_nd

def create_3d_pca_plot(safe_features_3d, unsafe_features_3d, feature_key):
    # Create 3D plot
...
def calculate_centroids(features_safe, features_unsafe):
    """
    Calculate centroids for safe and unsafe features
    
    Args:
        features_safe: Safe features in 3D PCA space
        features_unsafe: Unsafe features in 3D PCA space
        
    Returns:
        tuple: (safe_centroid, unsafe_centroid, safe_unsafe_centroid_euclidean_distance)
    """
    safe_centroid = np.mean(features_safe, axis=0)
    unsafe_centroid = np.mean(features_unsafe, axis=0)
    
    # Calculate Euclidean distance between centroids
    safe_unsafe_centroid_euclidean_distance = np.linalg.norm(safe_centroid - unsafe_centroid)
    
    return safe_centroid, unsafe_centroid, safe_unsafe_centroid_euclidean_distance
...
def plot_safe_unsafe_clusters(extracted_safe_unsafe_diffusion_features_per_sample):
    ...
    feature_key = 'up-level1-repeat1-vit-block0-cross-q'
    # Collect all safe and unsafe features for this feature key
    flattened_safe_features, flattened_unsafe_features = flatten_safe_unsafe_features(extracted_safe_unsafe_diffusion_features_per_sample, feature_key)

    # getting 3 components of PCA for 3D visualization
    pca_features,safe_features_3d, unsafe_features_3d = get_pca_features(flattened_safe_features, flattened_unsafe_features,n_components=3)

    create_3d_pca_plot( safe_features_3d, unsafe_features_3d, feature_key)

    safe_centroid, unsafe_centroid, safe_unsafe_centroid_euclidean_distance = calculate_centroids(safe_features_3d, unsafe_features_3d)
    ...
    create_gif_plot(feature_key, safe_features_3d, unsafe_features_3d, pca)
